...
SVL::Object::Object(const Renderer& renderer, VkCommandPool command_pool, SVL::Mesh & mesh, SVL::Texture & texture)
...
SVL::Object::Object(const Renderer& renderer, VkCommandPool command_pool, std::vector<Mesh> meshes, std::vector<Material> materials)
...
void SVL::Object::create_descriptor_sets(VkDescriptorPool descriptor_pool, std::array<VkDescriptorSetLayout, 2> layouts, Texture* environment)
{
	//set0
...
void SVL::Object::render(VkCommandBuffer command_buffer, std::array<VkPipeline, 2> pipelines, VkPipelineLayout pipeline_layout)
{
	...
	for (size_t i = 0; i < meshes.size(); i++)
	{
		...
		/*FragmentPushConstants fragment_push_constants{};
		fragment_push_constants.ambient = materials[meshes[i].material_id].properties.ambient;
		fragment_push_constants.diffuse = materials[meshes[i].material_id].properties.diffuse;
		fragment_push_constants.specular = materials[meshes[i].material_id].properties.specular;
		fragment_push_constants.opacity = materials[meshes[i].material_id].properties.opacity;
		if (!materials[meshes[i].material_id].properties.has_textures) fragment_push_constants.ignore_texture = true;
		if (materials[meshes[i].material_id].properties.has_opaque_tex) fragment_push_constants.has_opaque_tex = true;
		if (materials[meshes[i].material_id].properties.has_normal_tex) fragment_push_constants.has_normal_tex = true;
		if (materials[meshes[i].material_id].properties.has_ambient_tex) fragment_push_constants.has_ambient_tex = true;
		if (materials[meshes[i].material_id].properties.has_specular_tex) fragment_push_constants.has_specular_tex = true;
		vkCmdPushConstants(command_buffer, pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(fragment_push_constants), &fragment_push_constants);
		
		if (fragment_push_constants.opacity != 1.0f || fragment_push_constants.has_opaque_tex == 1.0f)//non-transparent
		{
			vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines[1]);//blend pipeline
		}
		else*/
		{
		...
		//vkCmdDrawIndexed(command_buffer, meshes[i].indices.size(), 1, 0, meshes[i].index_base, 0);
		vkCmdDrawIndexed(command_buffer, meshes[i].indices.size(), 1, meshes[i].index_base, meshes[i].vertex_base, 0);
	}
}

void SVL::Object::update_uniform(glm::mat4 projection, glm::mat4 view, std::array<SVL::PointLight, 4> lights, glm::vec4 view_pos)
{
...
void SVL::Object::set_ubo_model(glm::mat4 model)
{
	this->model = model;
}
